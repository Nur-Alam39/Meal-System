...
{	
	...
	{
		$query="select * from daily_deposite order by id";
		...
		while($data=mysqli_fetch_array($result))
		{
			...
			for($i=0;$i<30;$i++)
			{
				$deposite_per_member[$j]+=$data[$day_name[$i]];
			}
			$id=$data['id'];
			$name=$data['Name'];
	...
	if($_POST["action"] == "add")
	{
	  ...
	  $amount=$_POST["amount"]; 
	  $day=$_POST["day"];
	  $query = "update daily_deposite set $day =$day+'$amount' where Name='$name'";
	  mysqli_query($connect, $query);
	}
	if($_POST["action"] == "reset")
	{
	  $name=$_POST["name"];
	  $query = "update daily_deposite set Day01=0, Day02=0, Day03=0, Day04=0, Day05=0, Day06=0, Day07=0, Day08=0,Day09=0,Day10=0,
